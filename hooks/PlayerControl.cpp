    ...
    try {
        ...
        if ((IsInGame() || IsInLobby())) {
            ...
            if (State.ShowPlayerInfo && IsInLobby() && !State.PanicMode)
            {
                ...
                if (IsStreamerMode() || __this == *Game::pLocalPlayer || (isWhitelisted && State.HideWhitelistedPlayerInfo))
                    friendCode = "[Hidden]";

                if (isNameLocked) playerName = nameCheckerCol + playerName + "</color>";
                if (isBlacklisted) friendCode = playerIdCol + friendCode + "</color>";
                if (isWhitelisted) friendCode = levelCol + friendCode + "</color>";
            ...
            if (__this == *Game::pLocalPlayer) {
                ...
                // zoom code moved to InnerNetClient_Update hook for smoother zoom
            }
            ...
            // freecam code moved to InnerNetClient_Update hook for smoother camera movement
        }
        auto playerData = GetPlayerData(__this);
        ...
        /*if (!State.PanicMode && IsHost() && State.TournamentMode && !(State.DisableMeetings || (State.BattleRoyale || State.TaskSpeedrun)) && callId == (uint8_t)RpcCalls__Enum::ReportDeadBody) {
            for (auto p : GetAllPlayerControl()) {
                if (p != *Game::pLocalPlayer) {
                    ...
                    auto writer = InnerNetClient_StartRpcImmediately((InnerNetClient*)(*Game::pAmongUsClient), __this->fields._.NetId,
                        uint8_t(RpcCalls__Enum::SetName), SendOption__Enum::Reliable, __this->fields._.OwnerId, NULL);
